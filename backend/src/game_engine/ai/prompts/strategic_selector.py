...
## Win condition
You win when ALL of the opponent's cards are in their Break Zone. A sequence that breaks the opponent's last remaining card this turn is lethal - always select it over any non-lethal alternative.

## Charge and tussle basics
- Charge pays for playing cards, tussling, direct attacks, and activated abilities. Standard tussle/direct_attack cost is 2 Charge and activate_ability is 1 Charge - but some cards override these costs, or override normal tussle resolution entirely (e.g. an auto-win effect). Check the card_guidance section below for any such exception on a card actually in this game; don't assume the standard rule applies to every card.
- Tussle resolution (absent any card-specific override from card_guidance): the active player's attacking Toy gets a +1 Speed bonus for ordering. Whichever card is faster strikes first, dealing its Strength as damage to the other's current Stamina. If that drops the target to 0 Stamina or below, it breaks immediately. If it survives, it strikes back the same way before the exchange ends.
- A tussle being offered in a candidate sequence only means it is LEGAL, not that it is a good trade. Judge each one with the rule above (and any card_guidance override): does the attacker actually break the defender, before or without being struck back itself? If not, that tussle trades your card's health for nothing and should count against the sequence, not for it.
- Direct attack only works when the opponent has zero Toys in play, and breaks a random card from their hand.

## How to judge a candidate sequence
Every sequence shown to you already passed legality validation - your job is purely strategic ranking, not legality checking. Use the board legend below to resolve each sequence's target labels (e.g. Y1, O2) to the actual cards involved, and weigh whether the tussles/abilities in it are good trades by the rule above. Prefer, in this order: a lethal sequence > one that actually breaks the most real opponent threats > one that improves your board position or tempo without giving up cards for nothing > one that wastes the least Charge.
...
def generate_strategic_prompt(
    ...
    opp_broken = len(opponent.break_zone)
    opener_hint = ""
    if len(opponent.in_play) == 0:
        opener_hint = "With 0 opponent toys, a legal direct_attack line that breaks a card now usually beats a setup-only line."

    prompt = f"""<system>Select the best legal sequence.</system>

<goal>
Priority: lethal > removal > tempo > efficiency.
Lethal means breaking all {opp_remaining} remaining opponent cards this turn.
{opener_hint}
</goal>

<context>
phase={phase} turn={game_state.turn_number} your_charge={player.charge} your_toys={len(player.in_play)} opponent_toys={len(opponent.in_play)} opponent_remaining={opp_remaining} opponent_broken={opp_broken}
</context>
...
def convert_sequence_to_turn_plan(
    ...
    Args:
        ...
        trust_action_costs: When True, use each action's own ``charge_cost`` instead
            ...
            (dual) sequences, where a hallucinated charge_cost must not be trusted.
    ...
    for action in sequence.get("actions", []):
        ...
        # Safety check: Charge should never go negative
        ...
        if charge_after < 0:
            logger.warning(f"Charge went negative ({charge_after}) after {action_type} {card_name} - capping at 0")
            charge_after = 0
        ...
        charge_remaining = charge_after

    # Build TurnPlan
    total_charge_spent = sequence.get("total_charge_spent", 0)
    cards_broken = sequence.get("cards_broken", 0)
    ...
    return {
        ...
        "resources_summary": f"Charge: {player.charge}, Hand: {len(player.hand)} cards, In Play: {len(player.in_play)} toys",
        "sequences_considered": [sequence.get("tactical_label", "Unknown")],
        "selected_strategy": reasoning,
        "action_sequence": action_sequence,
        "charge_start": player.charge,
        "charge_after_plan": charge_remaining,
        "expected_cards_broken": cards_broken,
        "plan_reasoning": reasoning,
        "residual_charge_justification": None if charge_remaining < 2 else "Charge remaining but no valid attacks available",
    }
