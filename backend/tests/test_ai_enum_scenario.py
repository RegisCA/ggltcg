...
def test_turn1_surge_knight_enum(enum_planner):
    """Turn 1 Surge+Knight opener in enum mode: ≤1 Charge wasted, ≥1 broken."""
...
def test_turn2_aggressive_enum(enum_planner):
    """Turn 2 with a board in enum mode: efficient, no wasted-Charge regression."""
    setup, _ = create_game_with_cards(
    ...
    print(f"\n[enum] Turn2 charge_wasted={metrics.charge_wasted} broken={metrics.cards_broken}")
