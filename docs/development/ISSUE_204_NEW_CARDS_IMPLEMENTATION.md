...
**Branch:** `feature/issue-204-new-cards`

## Overview

This document provides a detailed analysis and implementation plan for adding 9 new cards to GGLTCG as specified in Issue #204. The cards have been added to `backend/data/cards_beta_20251206.csv` with preliminary effect definitions.

---

## Card Analysis Summary

| Card | Type | Cost | Effect Summary | Implementation Complexity | Priority |
|------|------|------|----------------|---------------------------|----------|
| **Surge** | Action | 0 | Gain 1 CC | 🟢 Trivial | 1 |
| **Dwumm** | Toy | 1 | Your cards have +2 speed | 🟢 Very Easy | 2 |
| **Twombon** | Toy | 1 | Your cards have +2 strength | 🟢 Very Easy | 3 |
| **Drop** | Action | 1 | Sleep a card in play (targeted) | 🟡 Easy | 4 |
| **Jumpscare** | Action | 0 | Return a card in play to owner's hand (targeted) | 🟡 Easy | 5 |
| **Sock Sorcerer** | Toy | 3 | Your cards are immune to opponent effects | 🟡 Medium | 6 |
| **VeryVeryAppleJuice** | Action | 0 | This turn, +1 all stats | 🟠 Medium-Hard | 7 |
| **Belchaletta** | Toy | 1 | Start of turn: gain 2 CC | 🟠 Hard | 8 |
| **Hind Leg Kicker** | Toy | 1 | When you play another card, gain 1 CC | 🔴 Hardest | 9 |
...
**Effect:** "Your cards have 2 more speed."  
**Effect Definition:** `stat_boost:speed:2`

**Implementation Status:** ✅ **COMPLETE** - Uses existing patterns!

**How It Works:**
- Uses existing `StatBoostEffect` class in `continuous_effects.py`
- Parser exists: `_parse_stat_boost()` - supports `speed`, `strength`, `stamina`, or `all`
...
**Effect:** "Your cards have 2 more strength."  
**Effect Definition:** `stat_boost:strength:2`
...
**Effect:** "Sleep a card that is in play."  
**Effect Definition:** Needs new type: `sleep_target:1` (1 target from all in play)
...
2. **New Parser:** `_parse_sleep_target()` in `effect_registry.py`

3. **CSV Update:** `sleep_target:1`
...
**Effect:** "Put a card that is in play into their owner's hand."  
**Effect Definition:** Needs new type: `return_target_to_hand:1`
...
2. **New Parser:** `_parse_return_target_to_hand()` in `effect_registry.py`

3. **CSV Update:** `return_target_to_hand:1`
...
**Effect:** "Your opponent's cards' effects don't affect your cards."  
**Effect Definition:** Needs new pattern: `team_opponent_immunity` (like Beary but for all your cards)
...
2. **Modify `is_protected_from_effect()`** in `game_state.py` to check all protection effects

3. **New Parser:** `_parse_team_opponent_immunity()` in `effect_registry.py`

4. **CSV Update:** `team_opponent_immunity`
...
**Effect:** "This turn, your cards have 1 more of each stat."  
**Effect Definition:** Needs new pattern: `turn_stat_boost:all:1`

**Challenge:** Effect must expire at end of turn (unlike Ka/Demideca which are permanent while in play).

**Implementation Approach:**

**Option A: Turn-Scoped Modification (Simpler)**
1. Add turn marker to player's modifications
2. Effect applies modification with turn number
3. Stat calculation ignores modifications from previous turns

**Option B: End-of-Turn Cleanup (More Complex)**
1. Apply modifications to all cards
2. Register cleanup callback for end of turn
3. Clean up modifications when turn ends
...
2. **Modify `get_card_stat()`** to include turn-scoped modifications

3. **Clear expired modifications** at start of turn or during stat calculation
...
**Effect:** "At the start of your turn, gain 2 charge."  
**Effect Definition:** Needs new pattern: `start_of_turn_gain_cc:2`

**Challenge:** Requires triggering effects at start of turn - mechanism partially exists in `TriggerTiming.START_OF_TURN` but isn't wired up.
...
2. **Modify `start_turn()` in `game_engine.py`:**
   ```python
...
3. **New Parser:** `_parse_start_of_turn_gain_cc()` in `effect_registry.py`

4. **CSV Update:** `start_of_turn_gain_cc:2`
...
**Effect Definition:** Needs new pattern: `on_card_played_gain_cc:1`

**Challenge:** Requires a trigger that fires when OTHER cards are played - new trigger type needed.

**Implementation Needed:**

1. **New TriggerTiming:** `WHEN_OTHER_CARD_PLAYED`

2. **New Effect Class:** `OnCardPlayedGainCCEffect` in `continuous_effects.py`
   ```python
...
3. **Modify `play_card()` in `game_engine.py`:**
   ```python
...
4. **New Parser:** `_parse_on_card_played_gain_cc()` in `effect_registry.py`

5. **CSV Update:** `on_card_played_gain_cc:1`
...
**Cards:** Surge, Dwumm, Twombon

1. ✅ Surge already works - just add test
2. Fix CSV effect definitions (remove `{}` placeholders)
3. Add AI prompt entries
4. Write tests for all three
5. Test manually against AI

### Phase 2: New Targeted Effects (Batch 2)

**Estimated Time:** 2-3 hours  
**Cards:** Drop, Jumpscare

1. Implement `SleepTargetEffect` class
2. Add `_parse_sleep_target()` parser
3. Implement `ReturnTargetToHandEffect` class  
4. Add `_parse_return_target_to_hand()` parser
5. Update CSV with proper effect definitions
6. Add AI prompt entries
7. Write tests
8. Test manually against AI
...
2. Update `is_protected_from_effect()` to check team immunity
3. Add `_parse_team_opponent_immunity()` parser
4. Update CSV
5. Add AI prompt entry
6. Write comprehensive tests (interactions with various effects)
7. Test manually against AI

### Phase 4: Turn-Scoped Effects (Batch 4)

**Estimated Time:** 3-4 hours  
**Cards:** VeryVeryAppleJuice

1. Design turn-scoped modification system
2. Implement `TurnStatBoostEffect` class
3. Modify stat calculation to include turn modifications
4. Add cleanup mechanism for expired modifications
5. Add parser
6. Update CSV
7. Add AI prompt entry
8. Write tests (including turn boundary tests)
9. Test manually against AI

### Phase 5: Triggered Effects (Batch 5)

**Estimated Time:** 4-5 hours  
**Cards:** Belchaletta, Hind Leg Kicker

1. Add `START_OF_TURN` trigger to game engine
2. Implement `StartOfTurnGainCCEffect` for Belchaletta
3. Add new `WHEN_OTHER_CARD_PLAYED` trigger timing
4. Modify `play_card()` to trigger on-play effects
5. Implement `OnCardPlayedGainCCEffect` for Hind Leg Kicker
6. Add parsers for both
7. Update CSV
8. Add AI prompt entries
...
2. **Effect application:** Effect works as expected
3. **Protection interactions:** Beary/Sock Sorcerer immunity respected
4. **Edge cases:** Empty targets, multiple instances, etc.

### Integration Tests

1. **Turn boundary tests:** Effects that span turns work correctly
2. **Trigger ordering:** Multiple triggered effects fire in correct order
3. **AI decision making:** AI correctly evaluates and uses new cards
...
2. **Create `HOW_TO_ADD_NEW_CARDS.md`** guide covering:
   - Using existing effect patterns
   - Creating new effect types
   - Adding AI awareness
   - Writing tests
3. **Update `README.md`** if card count changes significantly
...
2. Start with Phase 1 (easiest cards)
3. Test each batch locally before moving to next phase
4. Create PRs after each phase is complete
5. Document patterns for future card additions
