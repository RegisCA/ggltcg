...
This document summarizes the Google OAuth 2.0 authentication implementation for
GGLTCG.
...
- ✅ **Dependencies**: Added `google-auth`, `google-auth-oauthlib`, `PyJWT`,
`better-profanity`

- ✅ **User Model**: Created `UserModel` with Google ID, first name, custom
display name
...
1. Add routing (if not already present) with `react-router-dom`
1. Create a protected route component
1. Add login page to routes
...
1. Verify JWT token is included in API requests
...
1. **localStorage**: Tokens stored in localStorage (vulnerable to XSS)
1. **In-Memory Rate Limiting**: Rate limits reset on server restart (use Redis
for production)

1. **No Password Reset**: Not applicable (using Google OAuth)
1. **No Email Verification**: Handled by Google
1. **Single OAuth Provider**: Only Google (could add GitHub, etc.)
...
1. **Game Statistics**: Track wins, losses, cards played
1. **Leaderboards**: Rank players by wins and statistics
1. **Friend System**: Add/challenge friends
1. **Avatars**: Use Google profile pictures
1. **Refresh Tokens**: Implement for longer sessions
1. **Multiple OAuth Providers**: Add GitHub, Discord, etc.
1. **Admin Panel**: Manage users and content
1. **Player Achievements**: Badges and rewards
...
1. Review error logs in backend and browser console
1. Verify environment variables are set correctly
1. Test with curl commands to isolate frontend vs backend issues
1. Check rate limiting if getting 429 errors
...
- [@react-oauth/google Documentation](https://github.com/MomenSherif/react-
oauth)

- [PyJWT Documentation](https://pyjwt.readthedocs.io/)
- [OWASP Authentication Cheat Sheet](https://cheatsheetseries.owasp.org/cheatshe
ets/Authentication_Cheat_Sheet.html)
