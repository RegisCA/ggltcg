...
A simple web-based admin panel for viewing GGLTCG database data without complex authentication or heavy infrastructure.
...
| Endpoint | Purpose | Data Shown |
|----------|---------|------------|
| `GET /admin/stats/summary` | Dashboard stats | User count, game stats, AI log counts, recent activity |
| `GET /admin/ai-logs` | AI decision logs | Gemini prompts, responses, reasoning (limit: 50) |
| `GET /admin/ai-logs/{id}` | Specific AI log | Full prompt and response details |
| `GET /admin/games` | Games list | Active/completed games with metadata (limit: 100) |
| `GET /admin/games/{id}` | Full game state | Complete JSONB game state |
| `GET /admin/game-playbacks` | Completed games | Play-by-play summaries (limit: 30) |
| `GET /admin/game-playbacks/{id}` | Full playback | Starting decks, complete play-by-play |
| `GET /admin/players` | Player stats | Leaderboard-style player statistics |
| `GET /admin/users` | Registered users | Google OAuth user accounts |
...
**Access**: `http://localhost:5173/admin.html` (dev) or `/admin.html` (production)
...
| `games` | `/admin/games` | All game sessions |
| `game_playback` | `/admin/game-playbacks` | Completed games (24h retention) |
| `ai_decision_logs` | `/admin/ai-logs` | AI prompts/responses (1h retention) |
| `player_stats` | `/admin/players` | Aggregated player stats |
