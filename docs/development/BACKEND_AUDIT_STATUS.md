...
**Issue**: [#196 - Backend Audit](https://github.com/RegisCA/ggltcg/issues/196)  
...
|----------|-----------------|----------|--------|
| 1. ID-Based Lookups | 2 | 🔴 Critical | ✅ Fixed |
| 2. Method-Based State Modification | 1 | 🟡 Medium | ✅ Reviewed (Acceptable) |
| 3. GameEngine vs GameState Separation | 0 | - | ✅ Clean |
| 4. Owner vs Controller | 0 | - | ✅ Clean |
| 5. Effect System Architecture | 0 | - | ✅ Fixed (via #1.2) |
| 6. Single Source of Truth | 1 | 🟢 Low | ✅ Fixed (Dead code removed) |
| Documentation | 1 | 🟢 Low | ✅ Fixed |
...
- **Location**: `backend/src/game_engine/rules/effects/action_effects.py:337-341`
- **Issue**: Directly assigns `.speed`, `.strength`, `.stamina` to copy card
- **Analysis**: This is card **initialization** during Copy effect, not damage/modification during play. The Copy card is a fresh entity that needs its base stats set.
- **Decision**: **Acceptable** - This is initial setup, not runtime modification. No fix needed.
...
- `game_state.py:242-248` - `change_card_controller()` - Data operation for Twist
- `game_state.py:265` - `play_card_from_hand()` - Used by Beary's effect
- `player.py:70-84` - Helper methods for zone management

These are **data access layer** operations called by GameEngine. The logic lives in GameEngine.
...
The hardcoded card name checks in `turn_manager.py` were **dead code** (unused methods). They have been removed as part of Fix 6.1.

The active code in `game_engine.py` now uses proper effect type checking (`isinstance(effect, SetTussleCostEffect)`).
...
- **Issue**: `get_tussle_cost`, `calculate_card_cost`, and `can_raggy_tussle` were unused
...
| File | Change |
|------|--------|
| `game_engine.py` | Changed `target_name` → `target_id` in `calculate_card_cost()` |
| `game_engine.py` | Changed name check → `isinstance(effect, SetTussleCostEffect)` in `calculate_tussle_cost()` |
| `action_executor.py` | Added `target_id` to kwargs for Copy card support |
| `turn_manager.py` | Removed 3 unused methods (76 lines of dead code) |
| `routes_actions.py` | Fixed docstring to reference `card_id` not `card_name` |
...
The backend codebase is **cleaner than expected**. Most architectural principles are followed correctly:
