...
The `regisca-bot` GitHub account is used to create pull requests for **all agent-driven development work** (code generated by AI assistants, security updates, dependency upgrades, etc.) so that the main `RegisCA` account can review and approve them following proper PR workflow.

**Golden Rule**: If an AI agent wrote the code, use regisca-bot to create the PR. This applies to:
- Feature implementations done by AI agents
- Test file creation by AI agents
- Security updates (Snyk, etc.)
- Dependency upgrades
- Any other code changes where AI did the work

**Exception**: Use RegisCA only for manual changes you type yourself without AI assistance.
...
When creating pull requests for agent-driven work (AI-generated code, security updates, dependency upgrades, etc.):
...
For Snyk security updates, dependency upgrades, or any agent-driven code changes:
